...
///   • Recency weighting: logs from the last 7 days count 2×
...
///   • Peak risk window label (human-readable)
class HeatmapService {
  ...
  // ── Peak risk window label (human-readable) ─────────────────────
  /// Returns e.g. "9:00 PM – 11:00 PM" based on weighted heatmap.
  Future<String> getPeakWindowLabel() async {
    final weighted = await getWeightedHeatmap();
    if (weighted.isEmpty) return 'Insufficient data';
    final peak    = weighted.entries.reduce((a, b) => a.value > b.value ? a : b).key;
    final endHour = (peak + 2) % 24;
    return '${_fmt(peak)} – ${_fmt(endHour)}';
  }
