...
class _DailyCheckInCardState extends State<DailyCheckInCard> {
  ...
  Future<void> _loadCheckInStatus() async {
    ...
    if (mounted) setState(() { _checkedInToday = done; _loadingStatus = false; });
  }
    ...
    if (result == CheckInResult.confirmed) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: const Row(children: [
          Icon(Icons.local_fire_department_rounded, color: Colors.white, size: 20), // was 18
          SizedBox(width: 10),
          Expanded(child: Text(
  ...
  Future<void> _handleRelapse(BuildContext context) async {
    ...
    await widget.streakService.recordRelapse();
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: const Row(children: [
          Icon(Icons.favorite_border_rounded, color: Colors.white, size: 20), // was 18
          SizedBox(width: 10),
          Expanded(child: Text(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // ── Check-in button ──────────────────────────────
          if (_loadingStatus)
            const _LockedButton(label: 'LOADING...', icon: Icons.hourglass_empty_rounded)
            ...
            const _LockedButton(label: 'COME BACK TOMORROW', icon: Icons.lock_clock),
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: OutlinedButton.icon(
        onPressed: onPressed,
        icon: Icon(icon, size: 18, color: Colors.white.withOpacity(0.55)), // was 17
        label: Text(label,
            style: TextStyle(
